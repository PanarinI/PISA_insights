Три гипотезы для трёх исследовательских вопросов
Данные: PISA-2015
...
# Исследовательский вопрос 1. Существует ли связь между наличием классической литературы дома и количеством электронных книг в России?
# H0: статистически-значимой связи нет; H1: статистически-значимая связь есть

# Выберем переменные, проверим их тип: ST011Q07Ta (наличие classic literature) и ST012Q08NA (кол-во e-books)
...
# Обе переменные разделены по категориям. Это значит, что оптимально использовать метод хи-квадрат через таблицы частотности
...
# Статистический вывод: отвергаем H0 > существует значимая связь между наличием классической лит-ры и количеством эл. книг дома
# Создадим лист
...
#Исследовательский вопрос 2. Есть ли связь между возрастом, в котором учащийся прибыл в страну, где он сейчас учится, и уровнем его читательской компетенции?
# Выберем, посмотрим тип и переименуем переменные: ST021Q01TA (возраст, когда прибыл); READ (среднее по PV_READ'ам)
...
# У нас интервальная переменная reading и порядковая age_arrived. Соответственно, выбираем коэффициент Спирмена. Построим scatterplot чтобы визуализировать связь
...
# связь немонотонна, сперва положительна, потом отрицательна. Какой силы эта связь? Применим коэффициент корреляции Спирмена
...
# Коэф. корреляции = -0,13 (read), -0,11(math), -0.09(science): слабая отрицательная корреляция > отвергаем H0 (почему-то Пирсон дал более правдоподобый (как кажется) результат: -0,18(read))
...
# Содержательный вывод: имеется слабая связь между тем, насколько давно переехал учащийся в страну и уровнем его читательской компетенции.
# Ограничения: с 11 до 17 лет все выборки - маленькие (N<40), что может повлиять на корреляцию

### Иследовательский вопрос 3. Связано ли количество уроков в неделю по предметам (math - MMINS; science - SMINS; READ - LMEANS) с уровнем компетенции - read, science, math. Сравним связь в России и в Великобритании.
...
# Имеются сильные выбросы, обрежем их, стандартизуем шкалы и признаем распределениe MMINS и LMINS относительно-нормальным (вопрос - допустимо ли это в данном случае?):
...
# Результат: Корреляция имеется, отвергаем H0. 
# Содержательный вывод: количество часов занятий положительно коррелирует с баллом по предмету, однако по России видна парадоксальная обратная корреляция по русскому языку. Выдвигается гипотеза:
# переменная описана test language, возможно это часы подготовки к экзаменам, и если так, то, возможно, школьная подготовка к ЕГЭ по русскому не эффективна...
